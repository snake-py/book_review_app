...
const updateAvgRatingAdd = async (bookId, ratingByUser, update) => {
  ...
  const newAverageReview = Math.floor(
    (totalRating + ratingByUser) / (nrOfReviews + 1)
  );
  ...
  try {
    ...
    return {
      success: true,
      msg: `Updated the average rating ${newAverageReview}. Nr of Reviews. ${
        nrOfReviews + 1
      }`,
    };
  } catch (error) {
...
const updateAvgRatingDelete = async (bookId, ratingByUser) => {
  ...
  try {
    ...
    return {
      success: true,
      msg: `Updated the average rating: ${newAverageReview}`,
    };
  } catch (error) {
    return { success: false, msg: `msg: ${error}` };
  }
...
const upDateAvgRatingUpdate = async (
  bookId,
  oldRatingByUser,
  newRatingByUser
) => {
  ...
  if (nrOfReviews > 1) {
    newAverageReview = Math.floor(
      (totalRating - oldRatingByUser + newRatingByUser) / nrOfReviews
    );
  } else {
    newAverageReview = 1;
  }
