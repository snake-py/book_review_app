...
class BookController {
  async addBook(bookToAdd) {
    ...
    if (bookExist) {
      try{
          ...
          if((await didIncrement).success){
            return {status: 200, existing: true, msg: `The book with this isbn (${isbn}) is already in database ${(await didIncrement).msg}` };
          }else{
            return {status: 400, existing: true, msg: `The book with this isbn (${isbn}) is already in database. ${(await didIncrement).msg}`};
          }
      }catch(error){
    ...
    try {
        ...
        try{
          const didIncrement = await incrementClick(isbn);
          if((await didIncrement).success){
            return {status: 200, existing: false, msg: `The book with isbn ${newBook.isbn} is registered. ${(await didIncrement).msg}` };
          }else{
            return {status: 400, existing: false, msg: `The book with isbn ${newBook.isbn} is registered. ${(await didIncrement).msg}`};
          }
        }catch(error){
              return { status:400, existing: false, msg: `The book with isbn ${newBook.isbn} is registered. Error: ${error}` };
        }
    }catch (error) {
    ...
    if(likedPrev){
      try{
          ...
          if((await didDecrement).success){
            return {status: 200, liked: false, msg: `Removed the like. ${(await didDecrement).msg}` };
          }else{
            return {status: 400, liked: false, msg: `Like document removed. ${(await didDecrement).msg}`};
          }
      }catch(error){
        return {status: 400, liked: false, msg: `Delete failed with error: ${error}` };
      }
    //if not liked previously, then like
    //create a new like document  
    }else{
      const newLikeDoc = new Like({ user_id: newLike.user_id, book_id: newLike.book_id});
      try {
          await newLikeDoc.save();
          const didIncrement = await incrementLikeSum(book_id);
          if((await didIncrement).success){
            return {status: 200, liked: true, msg: `Liked. ${(await didIncrement).msg}` };
          }else{
            return {status: 400, liked: false, msg: `Like document added. ${(await didIncrement).msg}`};
          }
      } catch (error) {
  ...
  async getLikedBooks(user){
    try{
      ...
      return {status: 200, msg: `The user has liked ${likedBooks} books`, likedBooks: likedBooks};
    }catch(error){
