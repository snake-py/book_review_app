...
const {
  reviewValidation,
  updateAvgRatingAdd,
  updateAvgRatingDelete,
  upDateAvgRatingUpdate,
} = require("../helpers/ReviewControllerHelper");
...
class ReviewController {
  async addReview(inputReview) {
    ...
    if (!bookExist) {
      const newBook = new Book({
        ...
        author: author,
        thumbnail: thumbnail,
        like_sum: bookLikeSum,
        click_counter: bookClickCounter,
        average_rating: bookAverageRating,
      });
      try {
        await newBook.save();
      } catch (error) {
    ...
    if (isReviewed) {
      return {
        status: 200,
        success: false,
        msg: `The book with isbn ${isbn} is already reviewed by this user ${userId}`,
      };
    }
    //add the review
    try {
      ...
      if (result.success) {
        return {
          status: 200,
          success: true,
          msg: `The book with isbn ${isbn} is now reviewed by this user ${userId}. ${result.msg}`,
        };
      } else {
        return {
          status: 400,
          success: false,
          msg: `ERROR: ${(await result).msg}`,
        };
      }
    } catch (error) {
      return { status: 400, success: false, msg: `ERROR: ${error}` };
  ...
  async getReviews(user) {
    try {
      ...
      return {
        status: 200,
        msg: `The user has reviewed ${reviewedBooks.length} books`,
  ...
  async deleteReview(reviewToDelete) {
    try {
      ...
      const didChangeRating = await updateAvgRatingDelete(
        reviewToDelete.book_id,
      ...
      if (didChangeRating.success) {
        ...
        return {
          status: 200,
          success: true,
          msg: `${review.title} was deleted. ${didChangeRating.msg}`,
        };
      } else {
        return {
          status: 400,
          success: false,
          msg: `ERROR: ${didChangeRating.msg}`,
        };
      }
    } catch (error) {
      return {
        status: 400,
        success: false,
        msg: `Review was not found. ${error}`,
      };
    }
  }
  ...
  async updateReview(reviewToUpdate) {
    ...
    if (!reviewFound)
      return { status: 400, success: false, msg: "Review was not found" };
    ...
    try {
      ...
      await upDateAvgRatingUpdate(
        reviewToUpdate.book_id,
        reviewFound.rating,
        rating
      );
      ...
      return {
        status: 200,
        success: true,
        msg: `The review with id ${_id} is updated`,
      };
    } catch (error) {
